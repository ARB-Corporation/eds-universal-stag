...
export default async function decorate(block) {
  ...
  try {
    ...
    const url = new URL(window.location.href);
    const path = url.pathname.split('/').slice(3).join('/'); // fix
    ...
    const items = taglist.filter((eachList) => {
      ...
      const thirdSegment = segments[2] || ""; // safe access
    ...
    const uniqueTags = [
      ...new Set(
        items.flatMap((obj) => {
          if (!obj || typeof obj.tag !== "string") return []; // safety check
    ...
    console.error("Error while processing tags:", err);
  }
